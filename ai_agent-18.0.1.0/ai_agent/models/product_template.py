...
class ProductTemplate(models.Model):
    ...
    def setup_demo_mock_llm(self):
        """Create and confirm Demo Mock LLM records (no external API)."""
        IrModelData = self.env["ir.model.data"].sudo()
        for product in self:
            product.create_llm()
            llms = self.env["ai.agent.llm"].search(
                [("product_tmpl_id", "=", product.id)]
            )
            llms.write({
                "status": "confirmed",
                "ai_api_key": product.ai_api_key or "demo-mock-no-key",
            })
            chat_llm = llms.filtered(
                lambda llm: llm.model_id.name == "demo-gpt" and not llm.is_embedded
            )[:1]
            embed_llm = llms.filtered(lambda llm: llm.is_embedded)[:1]
            for xml_name, llm_rec in (
                ("demo_mock_llm_chat", chat_llm),
                ("demo_mock_llm_embed", embed_llm),
            ):
                if not llm_rec:
                    continue
                ...
                if existing:
                    existing.write({"res_id": llm_rec.id})
                else:
                    IrModelData.create({
                        "module": "ai_agent",
                        "name": xml_name,
