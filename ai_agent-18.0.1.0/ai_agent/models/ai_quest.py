...
class AIQuest(models.Model):
    ...
    def setup_demo_discuss_channels(self):
        """Create/pin Discuss chats for demo quests (chat + channel init types).

        Called from demo data so Playwright and new installs show bot threads in Discuss.
        """
        ...
        for quest in self:
            if quest.init_type == 'chat':
                if not quest.chat_user_id:
                    quest._onchange_init_type()
                bot = quest.chat_user_id
                if not bot:
                    _logger.warning(
                        "setup_demo_discuss_channels: no chat user for quest %s",
                        quest.display_name,
                    )
                    continue
                channel = DiscussChannel.with_user(admin).channel_get(
                    [admin.partner_id.id, bot.partner_id.id],
                    pin=True,
                    force_open=True,
                )
                quest.sudo().write({
                    'real_channel_id': channel.id,
                    'real_chat_user_id': bot.id,
                })
                if len(channel.message_ids) < 2:
                    post_ctx = {'ai_agent_skip_auto_reply': True}
                    channel.with_user(bot).with_context(**post_ctx).message_post(
                        body=_(
                            "Hi! I am your Odoo Demo Helper. Ask about AI Orchestration, "
                            "quests, or Odoo development (Demo Mock LLM — no API key)."
                        ),
                        message_type='comment',
                        subtype_xmlid='mail.mt_comment',
                    )
                    channel.with_user(admin).with_context(**post_ctx).message_post(
                        body=_(
                            "Hello! This is a demo chat for the USER_GUIDE screenshots."
                        ),
                        message_type='comment',
                        subtype_xmlid='mail.mt_comment',
                    )
            elif quest.init_type == 'channel':
                if not quest.channel_id:
                    quest._onchange_init_type()
                channel = quest.channel_id
                if not channel:
                    continue
                channel = channel.sudo()
                if admin.partner_id not in channel.channel_partner_ids:
                    channel.write({
                        'channel_partner_ids': [(4, admin.partner_id.id)],
                    })
                ...
                if not channel.message_ids:
                    channel.with_user(admin).with_context(
                        ai_agent_skip_auto_reply=True,
                    ).message_post(
                        body=_(
                            "Welcome to the demo team channel. Mention the bot to ask "
                            "payroll or Odoo legacy questions (Demo Mock LLM)."
                        ),
                        message_type='comment',
                        subtype_xmlid='mail.mt_comment',
                    )
    ...
    @api.model
    def setup_demo_sales_inventory_confirm_orders(self):
        """Confirm DEMO sale orders after demo XML load."""
        for xid in ("demo_so_acme", "demo_so_beta"):
            order = self.env.ref(f"ai_agent.{xid}", raise_if_not_found=False)
            if order and order.state in ("draft", "sent"):
                order.action_confirm()

    @api.model
    def setup_demo_sales_inventory_stock(self):
        """Set on-hand quantities for DEMO-* products (requires stock)."""
        ...
        if not location:
            return
        levels = {
            "demo_product_bracket": 250.0,
            "demo_product_chair": 45.0,
            "demo_product_desk": 18.0,
        }
        for xml_name, qty in levels.items():
            product = self.env.ref(f"ai_agent.{xml_name}", raise_if_not_found=False)
            if not product:
                continue
            quant = Quant.search([
    ...
    @api.model
    def setup_demo_sales_inventory_knowledge(self):
        """Embed Sales / Inventory demo records into AI Memory (FAISS)."""
        IrModelData = self.env["ir.model.data"].sudo()
        memory_xmlids = [
            "demo_memory_products",
            "demo_memory_sale_orders",
            "demo_memory_stock_quants",
        ]
        memories = self.env["ai.memory"]
        for xid in memory_xmlids:
            rec = IrModelData._xmlid_to_res_model_res_id(
                f"ai_agent.{xid}", raise_if_not_found=False
            )
            if rec and rec[1]:
                memories |= self.env["ai.memory"].browse(rec[1])
        if not memories:
            _logger.warning("setup_demo_sales_inventory_knowledge: no memory records")
            return
        embed = self.env.ref("ai_agent.demo_mock_llm_embed", raise_if_not_found=False)
        if embed and embed.status != "confirmed":
            embed.write({"status": "confirmed", "ai_api_key": "demo-mock-no-key"})
        memories.write({"status": "active"})
        for memory in memories:
            if not memory.ai_agent_llm_id and embed:
                memory.ai_agent_llm_id = embed.id
            memory.run()
            _logger.info(
                "AI Memory %s embedded (%s chunks in index)",
                memory.name,
                "yes" if memory.memory_faiss else "no",
            )

    @api.model
    def setup_demo_sales_inventory_quest(self):
        """Wire quest, discuss channel, and refresh knowledge after demo install."""
        self.setup_demo_sales_inventory_stock()
        self.setup_demo_sales_inventory_knowledge()
        quest = self.env.ref(
            "ai_agent.demo_quest_sales_inventory_chat",
            raise_if_not_found=False,
        )
        if quest:
            quest._onchange_init_type()
            quest.setup_demo_discuss_channels()
